  ...
  <div id="app">
    ...
    <div class="app-container">
      <div class="box">
        ...
        <!-- 新增标签弹层 -->
        <div class="add-form">
          <el-dialog title="新增简介" :visible.sync="dialogFormVisible">
            <template>
...
export default {
  ...
  methods: {
    ...
    handleEdit () {
      ...
      // 发送请求，提交表单数据
      this.$axios.post('/introduce/update', this.formData,
    ...
    handleAdd () {
      ...
      this.$axios.post('/introduce/add', this.formData,
        {
          headers: {
    ...
    handleUpdate (row) {
      ...
      // 发送请求，根据ID获取销售商数据
      this.$axios.get('/introduce/queryById?introduceId=' + row.id,
        {
          headers: {
            'content-type': 'application/json',
            token: this.token
    ...
    handleDelete (row) {
      this.$confirm('确认删除当前选中的记录吗?', '提示', { type: 'warning' }).then(() => {
        this.$axios.get('/introduce/delete?introduceId=' + row.id,
          {
            headers: {
              'content-type': 'application/json',
              token: this.token
